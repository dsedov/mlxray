...
float3 ray_color(Ray ray, const device float* geos, const device float* norms, const device float* bboxes, const device int* indices, uint seed) { 
    MetalRandom rand(seed);
    HitRecordStack hit_record_stack;
    hit_record_stack.count = 0;

    HitRecord hit_record = hit(ray, Interval{0.1, 10000.0}, geos, norms, bboxes, indices);
    if (!hit_record.hit) {
    ...
    for (uint i = 1; i < MAX_DEPTH; i++) {
        ...
        hit_record = hit(Ray{hit_record.p, direction}, Interval{0.0001, 10000.0}, geos, norms, bboxes, indices);
        if (hit_record.hit) {
            hit_record_stack.hit_records[hit_record_stack.count++] = hit_record;
        } else {
            break;

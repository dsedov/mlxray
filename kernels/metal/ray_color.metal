...
float3 ray_color(Ray ray, const device float* geos, const device float* norms, const device float* bboxes, const device int* indices, const device int* polygon_indices, MetalRandom rand, uint sample, uint samples) { 
    ...
    for (uint i = 1; i < MAX_DEPTH; i++) {
        float3 direction = rand.random_on_hemisphere(-hit_record.normal);

...
class ImageGeneratorThread(QThread):
    ...
    def __init__(self, shape):
        ...
        self.image_buffer = draw_circle(self.image_buffer, 512.0, 384.0, 30.0)
        self.image_buffer = draw_circle(self.image_buffer, 112.0, 384.0, 50.0)
    ...
    def run(self):
        ...
        while self.running:
            self.blur1_buffer = blur(self.image_buffer,1.0)
            self.blur2_buffer = blur(self.image_buffer,8.0)
...
def reaction_diffusion(a: mx.array, b1: mx.array, b2: mx.array):
    ...
    return outputs["out"]
def draw_circle(a: mx.array, x: float, y: float, r: float):
    source = """
        uint elem = (thread_position_in_grid.x + thread_position_in_grid.y * threads_per_grid.x) * 3;
        float dist = sqrt(pow(x - thread_position_in_grid.x, 2) + pow(y - thread_position_in_grid.y, 2));
        if (dist < r) {
            out[elem] = 1.0;
            out[elem + 1] = 1.0;
            out[elem + 2] = 1.0;
